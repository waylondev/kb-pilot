...
- **Own your own copy, pin consistency with tests** — Skills are published independently and never depend on a sibling at runtime. When two skills need the same deterministic routine (a heading rule, a drift check), each carries its own copy — and the cross-skill contract tests pin the copies behaviour-identical, so a change on one side turns red until the other follows. Duplication is accepted and *tested*, never a runtime coupling
