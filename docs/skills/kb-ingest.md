...
单遍扫描 source.md，遇 `#` 标题即创建节点，累积行号。纯正则解析，确定性输出，不调用 LLM。
...
这是唯一需要 LLM 参与的步骤。LLM 逐节点读取 source.md 对应行内容，生成：
...
source.md 大幅修改时，从 Step 5 重新执行（覆盖已有 tree.json）。注意：重建会覆盖已填充的 summary 和 keywords，需重新执行 Step 6。
