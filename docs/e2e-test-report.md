...
| "线程" → "Thread" | ❌ 不匹配 | ✅ 理解等价 |
| "风控" → "风险管理" | ❌ 不匹配 | ✅ 理解等价 |
| "预授权有效期" → "预授权交易" | ❌ 部分匹配 | ✅ 理解包含 |
| "GDP怎么算" → "GDP三种核算方法" | ❌ 不匹配 | ✅ 理解意图 |
| "信用卡审批流程" → "账户全生命周期" | ❌ 不匹配 | ✅ 理解层级 |

**结论**：keyword 匹配是字符串运算，LLM 语义理解是认知运算。用前者衡量后者，等于用尺子量温度。

### 3.5 纠错重复记录的设计立场

测试中 doc_001.jsonl 出现了 18 条记录，其中多条 correct_answer 相同（如"Docker 容器启动时间实测为 85ms"出现两次，来自不同 session_id 的并发纠正）。

| 设计选择 | 原因 |
|---|---|
| 不去重 | 重复记录代表多人共识，是可信度信号 |
| 不加权 | 加权是给 LLM 加拐杖，应由 LLM 自行判断 |
| 不加锁 | 并发写入冲突由 Git merge 解决，不在应用层处理 |

**关键立场**：corrections 是 Git 仓库文件，多人协作的合并冲突走 git merge 流程。应用层加锁是双重机制，违背"少即是多"。

### 3.6 大规模知识库的应对策略

当单库文档数超过几百篇时，**不在单库内做 manifest 分片或路由分层**——这违背"少即是多"的设计哲学。正确做法是按认知边界拆分 git 仓库：

| 策略 | 做法 | 适用规模 |
|---|---|---|
| 单库 | 一个 git 仓库，manifest 单文件 | 100-300 篇 |
| 分库 | 按团队/领域拆分多个 git 仓库 | 300+ 篇 |
| 跨库查询 | kb-chat 领域路由阶段选择目标仓库 | 多团队协作 |

**核心立场**：一个知识库 = 一个 git 仓库 = 一个认知边界。没有人会把所有领域的书塞进一个图书馆。规模问题应该用组织方式解决，而不是用工程方式解决。
...
当前 41 个用例的文档、题目、测试均由同一 LLM 实例完成，无法排除"LLM 已知答案"的偏差。盲测的目的是验证：**在 LLM 不知道答案所在位置的前提下，manifest.json + tree.json 的 metadata 是否足以引导 LLM 路由到正确位置。**

### 4.2 盲测设计原则

| 原则 | 说明 |
|---|---|
| 新会话 | 在全新 chat session 中执行，不带历史上下文 |
| 人工出题 | 题目由人工准备，不告诉 LLM 答案在哪 |
| 全链路 | kb-chat 走完整 6 步，记录每步中间结果 |
| 可对比 | 人工标注标准答案，与 LLM 路由结果逐项对比 |

### 4.3 盲测流程

```mermaid
flowchart TD
    P1["人工准备 50 题<br/>+ 标准答案 doc_id/ch_id/行号"] --> P2["新会话执行 kb-chat<br/>记录每步中间结果"]
    P2 --> P3{"对比标准答案"}
    P3 -->|"doc_id 命中"| D1["文档路由准确"]
    P3 -->|"ch_id 命中"| D2["章节定位准确"]
    P3 -->|"行号范围命中"| D3["内容截取准确"]
    D1 --> R["生成盲测报告"]
    D2 --> R
    D3 --> R
...
| 维度 | 评分 | 说明 |
|---|:---:|---|
| 文档路由 | 0/1 | 命中标准 doc_id 得 1 分 |
| 章节定位 | 0/1 | 命中标准 ch_id 或其父节点得 1 分 |
| 答案正确性 | 0/1 | 答案与标准答案一致得 1 分 |

**关键**：文档路由和章节定位是独立评分维度。即使答案正确，如果路由错了文档，说明是"蒙对的"，不应计满分。

### 4.5 题目准备指南

- **覆盖 20+ 领域**：从 52 个领域中抽 20 个，每领域 2-3 题
- **题型分布**：简单问答 60% + 模糊匹配 15% + 跨文档对比 15% + 纠错 10%
- **避免暗示**：题目不包含 doc_id、文件名、章节标题等暗示信息
- **可验证**：每题必须有明确的标准答案和行号范围

题目格式（人工准备，不放入知识库）：

```
Q1: 信用卡预授权交易的有效期是多少天？
标准答案: doc_214, ch_2, L117-L126, 5天
Q2: ...
```

### 4.6 执行步骤

1. 人工准备 50 题及标准答案（独立文件，不进入 knowledge_repo）
2. 开启全新 chat session
3. 逐题执行 `Use Skill: kb-chat {问题}`
4. 记录每题的：命中 doc_id、命中 ch_id、引用行号、最终答案
5. 与标准答案对比，统计三段评分
6. 生成盲测报告（准确率 + 失败案例分析）

### 4.7 盲测执行结果

**已执行**。2026-08-02 完成路由可达性盲测，详细结果见 [blind_test_results.md](blind_test_results.md)，题目见 [blind_test_questions.md](blind_test_questions.md)。

#### 执行方式

- 题目由主会话基于 tree.json 的 summary + keywords 构造（不读原文细节），记录标准答案后写入独立文件
- 执行由独立 subagent 完成，**不带主会话上下文**，未参考标准答案
- subagent 独立读取 manifest.json + tree.json + source.md 走完整 6 步流程

#### 盲测结果

| 维度 | 准确率 | 说明 |
|---|:---:|---|
| 文档路由 | **10/10** | 全部命中目标 doc_id |
| 章节定位 | **10/10** | 全部命中目标 ch_id（含 1 题命中父节点后二次定位） |
| 行号命中 | **10/10** | 引用行号全部与标准范围重叠 |
| **综合** | **30/30** | 三级索引路由可达性全部通过 |

#### 题目覆盖

10 题覆盖 10 个领域：美食/建筑/军事/哲学/经济/化学/生物/物理/天体物理/财经

| # | 问题 | 命中 doc_id | 命中 ch_id | 行号 |
|:---:|------|:---:|:---:|:---:|
| 1 | 咖啡杯测时主要评估哪些维度？ | doc_009 | ch_4_1 | L65-L79 |
| 2 | LEED认证主要考察哪些方面？ | doc_026 | ch_2_1 | L18-L27 |
| 3 | 各国洲际导弹的射程和精度对比？ | doc_051 | ch_3_1 | L27-L34 |
| 4 | 功利主义和义务论的核心分歧是什么？ | doc_058 | ch_1_1 | L5-L12 |
| 5 | CPI和PPI在衡量通胀时有什么区别？ | doc_091 | ch_2_1 | L25-L33 |
| 6 | 稀土元素在工业上有哪些关键应用？ | doc_096 | ch_2_1 | L16-L25 |
| 7 | 基因编辑技术面临哪些伦理争议？ | doc_101 | ch_3_1 | L29-L36 |
| 8 | 量子力学有哪几种主要解释？ | doc_106 | ch_2_1 | L17-L26 |
| 9 | 黑洞有哪些不同类型？ | doc_110 | ch_2_1 | L17-L25 |
| 10 | 信用卡系统的实时风控如何运作？ | doc_214 | ch_4 | L265-L332 |

#### 关键发现

1. **歧义排除能力**：subagent 独立识别并排除了 2 处干扰项——"稀土"在 doc_073 世界经济地理仅作为矿产 tag（无工业应用内容）；"洲际"在 doc_087 酒店文档指"洲际酒店集团"（非洲际导弹）。这证明 LLM 语义理解能处理同形歧义，不需要额外的消歧机制。

2. **纠错相关性判断**：4 个候选文档存在 corrections 文件（doc_009/doc_091/doc_101/doc_214），subagent 全部正确判断为"与本问题不相关"，未影响答案。其中 doc_214 的 10 条并发纠错记录（围绕审批时效）被正确识别为属于 ch_1 范畴，与 ch_4 风控问题无关。

3. **大文档二次定位**：Q10 的 tree.json 中 ch_4 无子节点，subagent 命中父节点后，通过源文 `### 4.1 实时风控` 子小节标题二次精确定位到 L267-L286，证明大文档（682 行）的章节压缩不影响定位精度。

4. **metadata 足以引导路由**：10 题的题目构造仅基于 tree.json 的 summary + keywords，未读原文细节。subagent 仅凭 manifest.json + tree.json 的 metadata 即可完成全部路由，验证了"目录 + 原文"设计的核心假设。

#### 局限说明

本次盲测验证的是**路由可达性**（metadata 是否足以引导路由到正确位置），不是**答案正确性**（答案是否与人工标注一致）。subagent 虽不带主会话上下文，但仍为同一 LLM 实例，权重中的知识无法消除。严格的答案正确性验证仍需人工出题 + 人工评分。