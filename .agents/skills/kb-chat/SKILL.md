...
One deterministic check backs Step 3. It lives in the **kb-ingest** skill directory (`{ingest_dir}/scripts/check_source.py`) and is read-only, so kb-chat can run it without writing anything:
