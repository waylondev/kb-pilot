...
- **Table-only documents need a derived H2** (field-tested: CSV, single-sheet XLSX): kb-ingest's tree starts at H2, and `no_headings` fires when a document has only an H1 + a table. There is no section title in the source to fix this with — so add **one H2 derived from the file name / table-header semantics** (a merchant CSV → `## Merchant directory`). This is structural normalisation, not a content edit; keep the title generic and checkable against the header row, and note the addition in your report
