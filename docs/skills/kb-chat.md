...
基于结构化树索引（tree.json）和全文（Markdown 原文）的知识库问答。处理知识问答、自我纠错、跨文档对比、知识同步、路由偏好记忆。
...
Step 3: 章节定位
  ...
  └── 取匹配数最多的节点
    ...
    ▼
Step 4: 内容截取
  ├── 按 start_line / end_line 读取 Markdown 原文
  └── 超过 2000 字时二次精确定位
    │
    ▼
Step 5: 纠错加载
...
根据选中节点的 `start_line` 和 `end_line`，从文档原文中精确截取对应行。内容超过 2000 字时，启用二次定位（在截取范围内进一步搜索关键词）。
