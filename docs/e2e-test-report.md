...
> 测试日期：2026-08-01 | 知识库规模：52 领域 × 213 篇文档 | 32 用例 | 准确率 100%
...
| 领域数 | 52 |
| 文档总数 | 213 |
| 最大文档行数 | 600 行（doc_213 Python 标准库速查手册） |
| tree.json 填充率 | 100%（全部由 LLM 填充 summary/keywords） |
| manifest.json 条目 | 213 |
...
1. **LLM 为核心的设计正确**：tree.json + manifest.json 的确定性路由在 213 篇文档、52 个领域规模下保持 100% 准确率，且不受规模增长影响。超大型文档（600 行）的自动节点压缩不影响路由精度。
...
6. **超大型文档无性能衰减**：600 行文档的 tree.json 压缩为 2.1 KB，LLM 路由准确率保持 100%，与 20 行文档无差异。

7. **零基础设施不牺牲能力**：纯文件系统 + Git 的方案在精准度、可追溯性、协作能力上均优于需要重型基础设施的替代方案。

### 4.2 局限

- 32 个用例覆盖 22 个领域，30 个领域未测试
...
- 超大型文档（>10 万行）的 tree.json 解析性能测试